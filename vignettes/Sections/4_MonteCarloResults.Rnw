...
@
\noindent The intuition behind this bias is given in the example in Section \ref{Section:Example} and a formal treatement is available in \citet{Klein2015a}.
From Eqn \ref{Eqn:ExampleOutcomeEqn}, we know that \code{epsilon = delta*eta}. Thus, conditional on \code{eta}, the unobservables in the outcome equation are independent of the exogenous variables (because \code{xi} does not enter the selection equation). 
